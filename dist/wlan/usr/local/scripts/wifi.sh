...
case "$2" in
  ...
  rate)
    if [ -z "${3:-}" ]; then
        ...
        echo ""
        echo "Note: Connected-state SET is unsupported. Configured values are applied before association"
        echo "      on boot, and only while rate_adapt.enabled is true - when it is false the boot path skips"
        echo "      the SET, so configured and live can stay different across reboots."
        echo "      fwcfg_watch logs later changes of the live value to syslog while logger.fwcfg_watch_sec > 0"
        echo "      (0 turns it off); it never compares live against JSON."
        if [ "$IFACE" = "mlan0" ]; then

...
wpa_cli status 폴링(COMPLETED@target) 확인 시에만 성공(exit 0, notify)이어야 한다.
cross-SSID(wifi connect 래퍼) 경로는 종전 계약(returncode) 유지.
"""
import sys
import os
...
def _setup(monkeypatch):
    ...
    monkeypatch.setattr(passive_roam.time, "sleep", lambda *_: None)
    return notify
...
def test_same_ssid_fail_exit0_is_failure(monkeypatch):
    ...
    with patch.object(passive_roam.subprocess, "run", return_value=_Run(0, "FAIL\n")), \
         patch.object(passive_roam, "get_associated_bssid") as gab:
...
def test_same_ssid_ok_not_confirmed_is_failure(monkeypatch):
    ...
    with patch.object(passive_roam.subprocess, "run", return_value=_Run(0, "OK\n")), \
         patch.object(passive_roam, "get_associated_bssid", return_value=FROM), \
         patch.object(passive_roam.time, "monotonic", side_effect=[0.0, 0.0, 10.0]):
        assert roam_to_ap(IFACE, _ap(), current_ssid="Net") == 1
    notify.assert_not_called()
...
def test_same_ssid_ok_and_confirmed_is_success(monkeypatch):
    ...
    with patch.object(passive_roam.subprocess, "run", return_value=_Run(0, "OK\n")), \
         ...
         patch.object(passive_roam.time, "monotonic", return_value=0.0):
...
def test_same_ssid_confirm_waits_for_target(monkeypatch):
    notify = _setup(monkeypatch)
    with patch.object(passive_roam.subprocess, "run", return_value=_Run(0, "OK\n")), \
         patch.object(passive_roam, "get_associated_bssid", side_effect=[FROM, TARGET]) as gab, \
         patch.object(passive_roam.time, "monotonic", return_value=0.0):
        assert roam_to_ap(IFACE, _ap(), current_ssid="Net") == 0

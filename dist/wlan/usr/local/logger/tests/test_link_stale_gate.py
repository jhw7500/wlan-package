...
종전 get_link_info 는 mtime 불변이면 캐시를 무기한 반환해, 생산자가 hang
...
def test_stale_returns_none_and_warns_once(_globals):
    ...
    assert wifi_roam.get_link_info() is None, "stale 파일이 판정에 사용됨"
    assert len(_warn_calls()) == 1
    # 같은 에피소드 반복 호출 — 경고 중복 발행 금지
    assert wifi_roam.get_link_info() is None
    assert len(_warn_calls()) == 1
...
def test_stale_bypasses_mtime_cache(_globals, monkeypatch):
    ...
    assert wifi_roam.get_link_info() is not None   # 캐시 프라임(fresh)
    real_time = time.time()
...
def test_recovery_resets_warning(_globals):
    ...
    assert wifi_roam.get_link_info() is not None

...
- **기존 기기는 바뀌지 않는다.** `DEBIAN/postinst` 의 JSON deep merge 는 "새 키는 기본값에서 추가, 기존 키는 사용자 값 유지" 규칙이고 `low_thresh`/`high_thresh` 는 이미 모든 배포 기기에 존재한다. 마이그레이션은 **의도적으로 넣지 않았다** — 기존 기기는 이미 실질적으로 `40/65` 로 동작하고 있어 동작상 이득이 없고, 사이트가 의도적으로 넣은 값을 덮어쓸 위험만 남기 때문이다.
- **기존 기기를 전환하려면** 운용자가 `wifi <iface> rate 1 40 65 100` 으로 JSON 을 바꾸고 **재부팅**한다(적용 시점이 boot 이고 연결 중 SET 은 먹지 않는다). 전환하지 않아도 동작은 동일하며, 설정 표기가 실제 동작과 다르게 남는 것이 유일한 차이다.

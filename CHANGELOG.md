...
> SemVer **patch** — 출하 `rate_adapt` 기본값을 `70/90` 에서 `40/65` 로 바꾼다. 근거는 regime 별로 세기가 다르다 — 가벼운 트래픽에서는 FW 가 `70/90` 을 트래픽 2~3 초 뒤 `40/65` 로 덮어쓰고 그 값이 실사용 상태이므로 **두 설정의 귀결이 같다**(전환 관측, 확정). 무거운 트래픽에서는 `70/90` 이 살아남고 선택 rate 가 45.53 vs 45.65 Mbps 로 나왔으나 **그 비교는 미해결**이다 — 설정당 1 회 측정이었고 허용오차 1.39 가 잡음에 대해 검증되지 않았다(아래 2026-09-14 정정). 기본값 변경은 **light regime 근거만으로** 유지된다. **신규 설치·factory reset 한정** — 기존 기기는 의도적으로 손대지 않는다.

### FW 가 정적 rate_adapt pair 를 덮어쓴다 (실측)

- 캠페인 3건, 총 17 arm, 부팅마다 fresh FW 다운로드, 배경 WLAN 서비스 9종 정지, 60 초 트래픽, `rate_adapt_cfg` 를 50ms 로 조밀 폴링했다. 측정 요약은 `docs/rate_adapt_cfg_investigation_2026-09-12.md` §12. 원시 trace 와 arm 별 audit 는 빌드 호스트의 `release/rate-*-20260914-*/` 에만 있고 저장소에는 넣지 않는다(`release/` 는 gitignore).
- **트리거는 association 이 아니라 집계 Tx 트래픽**이다. 송신 개시 2~3 초 후 50~60ms 창 안에서 단일 계단으로 바뀐다. 2026-08-31 재검증이 재현하지 못한 이유가 이것이다 — 당시 자극은 association 5 회뿐이었다.
- **설정 `LOW` 가 지배하고 `HIGH` 는 무관**하다. `HIGH=90` 이 전환 arm(`70/90`)과 무전환 arm(`58/90`·`55/90`·`50/90`) 양쪽에 나타난다.
- **경계는 링크 상태에 따라 움직인다.** 가벼운 트래픽에서 `58<LOW<=60`, 몇 시간 뒤 더 나쁜 링크에서 58 이하, 무거운 트래픽(1000pps/1400B)에서 `75<LOW<=80`. 고정 상수가 아니다.
- **도착값은 대개 `40/65` 다** — 그러나 불변은 아니다. 출발값 5 종(`70/90`x14, `58/90`, `60/80`, `65/90`, `80/90`)에서 관측된 전환 18 회는 전부 `[40,65]` 였으나, 후속 시험에서 `60/90` 이 **`[30,50]`** 으로 착지하는 반례가 나왔다(아래 정정 참조).
- **덮어쓴 값은 실사용 상태다.** `mlanutl getdatarate` 를 200ms 로 표본화해, `70/90` 으로 설정해 `40/65` 로 덮어써진 인터페이스의 선택 전송 rate 평균이 58.66 Mbps 로 진짜 `40/65` 설정(58.11)과 구분되지 않고 `40/90`(53.95)과는 명확히 달랐다(사전 선언 허용오차 1.39, MCS 히스토그램도 독립적으로 일치). 표시만의 문제가 아니다.
- **dynamic(`0xff/0xff`)과 FW 기본값 상태는 면제**다. SET 을 한 번도 받지 않은 상태는 967/967 표본 전부 dynamic 을 유지했다. 정적 pair 만 덮어써진다.
- **무거운 트래픽에서는 `70/90` 이 덮어써지지 않는다**(경계가 `75<LOW<=80`, heavy arm 0/2 전환). 그 regime 에서 두 설정을 직접 비교한 결과 **선택 rate 가 구분되지 않았다** — `70/90` 45.5300 Mbps {MCS4:108, MCS5:109} vs `40/65` 45.6498 Mbps {MCS4:106, MCS5:111}, 차이 0.1198 Mbps 로 사전 선언한 허용오차 1.39 Mbps 의 약 1/12 다. 두 arm 모두 전환 0 회로 각 설정이 창 전체에서 live 였다(PR #332 Codex P1 지적에 대한 실측 답변). **[정정 2026-09-14, PR #335 Codex P1]** 이 항목은 원래 이를 "regime 별로 각각 확인된 중립"이라고 불렀다. 그 표현은 과대주장이다 — 이 비교는 설정당 1 회 측정이고 허용오차 1.39 가 잡음에 대해 검증되지 않았다. 설정 내 산포를 재 보니(5 번째 캠페인) light 프로파일에서 최대 3.83 Mbps 였고, 이는 관측된 차이 0.1198 보다 훨씬 크다. 따라서 heavy regime 비교는 **미해결(반복 측정 대기)** 이며 두 설정이 같다는 증거도 다르다는 증거도 없다. 상세는 조사 문서 §12.11.
- **연결 중 재설정은 불가**하다. 연결 상태 SET 은 출력만 바뀌고 즉시 GET 은 덮어쓴 값을 그대로 반환한다(벤더 계약도 pre-association 한정). 따라서 모니터링 기반 자동 교정은 성립하지 않는다 — `fwcfg_watch` 는 관측·경고 용도로만 쓴다.

### 출하 기본값 `40/65` (신규 설치 한정)

- `dist/wlan/opt/wlan/config/wifi_init_conf.json` 의 `mlan0`·`mlan1` `rate_adapt.low_thresh` 70→40, `high_thresh` 90→65. `mode`(1)·`interval_ms`(100)·`enabled`(true) 는 그대로다. `docs/wifi_init_conf.schema.json` 의 해당 `default` 4 개도 같이 옮겼다(값 범위 `0..100|255` 는 변경 없음, 검증기의 `low<high` 요구도 충족).
- **기존 기기는 바뀌지 않는다.** `DEBIAN/postinst` 의 JSON deep merge 는 "새 키는 기본값에서 추가, 기존 키는 사용자 값 유지" 규칙이고 `low_thresh`/`high_thresh` 는 이미 모든 배포 기기에 존재한다. 마이그레이션은 **의도적으로 넣지 않았다** — 부트 게이트 조건의 light arm 에서는 귀결이 같았고(15/15, 중간 트래픽은 arm 1 개), heavy 트래픽에서는 `70/90` 이 살아남으므로 차이가 있을 수 있으나 **그 차이를 우리가 측정하지 못했다**. 경계 하한이 미측정이므로 경계가 70 위에 앉는 세션에서는 light 에서도 동작 변경이다(아래 2026-09-14 정정, 조사 문서 §12.8). 즉 근거 없이 사이트가 의도적으로 넣은 값을 덮어쓰지 않는다는 뜻이다. 종전에 적었던 "기존 기기는 이미 실질적으로 `40/65` 로 동작하고 있어 이득이 없다"는 heavy regime 에서 거짓이므로 철회한다.
- **기존 기기를 전환하려면** 운용자가 `wifi <iface> rate 1 40 65 100` 으로 JSON 을 바꾸고 **재부팅**한다(적용 시점이 boot 이고 연결 중 SET 은 먹지 않는다). light/중간 트래픽에서는 전환하지 않아도 동작이 같고 설정 표기만 실제와 달라진다. heavy 트래픽에서는 `70/90` 이 살아남으므로 **동작이 다를 수 있으며 그 차이는 미측정**이다.
- 근본 수정은 FW 몫이다. 벤더 요청서는 `docs/rate_adapt_cfg_nxp_request.md` 에 질문 8 개로 정리돼 있다(무엇과 비교하는지, `40/65` 가 명명된 프로파일인지, 정적 pair 가 override 되는 것이 의도인지 등).

### 정정 (2026-09-14 후속 시험, 4 번째 캠페인)

`release/rate-txpwr-inverse-20260914-c38b243a` (4 arm, 부팅마다 fresh FW, 연결 전 부트 드롭인 게이트,
Tx 전력을 레버로 사용). 위 절의 두 주장을 정정한다.

- **착지값 불변은 거짓이다.** `60/90` 이 단일 계단으로 **`[30,50]`** 에 착지했다(50ms 폴링 1009 표본,
  `[40,65]` 경유 없음). 전 캠페인 표본 시퀀스 전수 재계수 결과 48 arm 중 전환 21 회이고
  `[40,65]` 20 회 + `[30,50]` 1 회, **21 회 모두 단일 계단**이다. 따라서 `[40,65]` 는 지배적 착지값일
  뿐 유일한 값이 아니다.
- **`HIGH` 무관은 전환 여부에만 적용된다.** 같은 `LOW=60` 이 `HIGH=80` 에서는 `[40,65]`,
  `HIGH=90` 에서는 `[30,50]` 로 갔다. 착지값에는 `HIGH` 가 관여한다.
- **2026-08-31 의 `30/50` 관측은 실재였다.** 아래 "문서 정정" 절과 가이드 §11.5 는 그 관측을
  "미재현"으로 설명해 치웠으나, `30/50` 은 재현됐다. 당시 자극(association 5 회)이 트리거를
  만들지 못했을 뿐 값 자체는 허구가 아니었다.
- **경계가 링크 품질을 따른다는 첫 통제 인과 증거.** 같은 `LOW=60` 에서 기본 전력(재시도/MPDU
  0.7225)은 전환했고 2 dBm(0.0050, 약 145 배 개선)은 전환하지 않았다. 즉 경계가 기본 전력의
  `<60` 에서 2 dBm 의 `60<LOW<=70` 으로 **올라갔다**. 종전 근거는 "몇 시간 뒤 더 나쁜 링크"라는
  기회적 관측뿐이었다.
- **부수 관측**: 이 근거리 링크(-48 dBm)에서 Tx 전력 저감은 링크를 **개선**한다(기본 20 dBm 이
  최악: 재시도/MPDU 0.68~0.72 vs 2 dBm 0.005~0.013). 20/2/20/2 교대 실행으로 순서 교란을 배제했다.
- **설명되지 않은 사실**: `LOW=60` 이 `LOW=70` 보다 **더 빨리** 전환했다(0.24s vs 2.90s). 낮은 임계가
  만족하기 쉬우므로 직관과 반대다. 자체 모델은 이미 2 건 반증됐으므로 수정 모델을 제안하지 않고
  벤더 질문으로 넘긴다.

출하 기본값 `40/65` 자체는 유지한다 — 이 정정은 착지값·`HIGH` 적용범위에 대한 것이고, light regime 의 전환 관측(덮어써지므로 귀결이 같다)에는 영향이 없다. heavy regime 비교의 상태는 아래 2026-09-14 정정을 참조한다.

### 현장 튜닝 가능값 검증 (2026-09-14, 5 번째 캠페인)

`release/rate-tune-4560-20260914-384f81e5` (8 arm, 4 설정 x 2 반복, arm 마다 fresh FW, 연결 전 부트 드롭인 게이트,
light 프로파일 60 초 창, `getdatarate` 200ms 표본화). 출하 기본값은 `40/65` 를 **유지**한다.

- **`45/60` 과 `50/70` 은 시험한 랩 링크에서 덮어써지지 않았다.** 각 2 회 독립 부팅에서 전환 0 회로 재현됐다.
  이로써 `LOW<=55` 설정의 무전환 arm 이 **15 개**가 됐다 — `40/65` x4, `40/90` x3, `45/60` x2,
  `50/70` x4, `50/90` x1, `55/90` x1 (합 15, 이 중 1 개만 heavy 프로파일). 같은 조건에서 전환이
  관측된 arm 은 0 개다. `45` 는 이번에 처음 시험된 값이다.
  **이것은 현장 보증이 아니다** — 경계 하한이 미측정이므로(아래 주의 항목) 랩보다 나쁜 현장
  링크에서는 덮어써질 수 있다. 조사 문서 §12.11 과 같은 범위 한정을 쓴다.
- **성능 차이는 이 계기로 분해되지 않았다.** 사전 등록한 계기 교정이 실패했다 —
  같은 설정의 두 측정 산포(최대 3.8326 Mbps, `45/60`)가 기준 설정쌍 간격(`40/65` vs `40/90`
  = 3.4684 Mbps)보다 크다. 따라서 설정 간 성능 차이를 주장하지 않는다. 분해에는 창을
  300 초 이상으로 늘리고 설정당 3 회 이상 반복해야 하며 약 2 시간이 든다.
- **출하 기본값을 `40/65` 로 유지하는 근거**: 성능으로는 후보들을 가릴 수 없으므로 남는 근거가
  ⓐ 관측 최저 경계(58) 대비 여유가 가장 크고(18 포인트 vs `45/60` 13, `50/70` 8),
  ⓑ FW 자신의 착지값이어서 설정 표기와 실제 동작이 일치하고, ⓒ 설정 내 산포가 가장 작다
  (0.4376 vs `45/60` 3.8326)는 것이다. `45/60` 은 측정으로 뒷받침되지 않는 동작점이다.
- **주의**: 경계 하한은 여전히 미측정이다. 이 랩 링크에서 무전환이라는 사실이 더 나쁜 현장
  링크에서의 무전환을 보장하지 않는다(Tx 전력으로는 링크를 악화시킬 수 없어 시험 불가).

### 문서 정정

- `docs/wifi_init_conf_guide.md` §11.5 에 2026-09-14 정정 노트를 추가한다. 기존 2026-08-31 노트("30/50 복귀 미재현")는 **그 자극(association 5 회)에 대해서만** 사실이므로 지우지 않고 남기되, 값 자체를 부정하는 뜻이 아니라는 단서를 붙인다 — `30/50` 은 후속 시험에서 재현됐다(위 정정 참조).
- `docs/wifi_init_conf_webui_handoff.md` 의 `rate_adapt.low_thresh`/`high_thresh` 기본값 행과 `interval_ms` 행의 트리거 설명을 고친다.
- 코드·테스트의 `70/90` 은 그대로 둔다. `scripts/qa/test_fwcfg_watch.py` 의 `RATE_ADAPT_STATIC` 은 mlanutl 출력 파서를 고정하는 실측 샘플이고, `wifi_fw_config_test.sh` 의 `70/90` 은 자체 fixture 다 — 둘 다 출하 기본값과 무관하다.
...
- **적용 범위는 신규 설치뿐이다.** 이 분기는 `id admin` 이 실패할 때만 타므로, 이미지가 계정을 만들었거나 이미 존재하는 기기는 `already exists; left untouched` 로 빠지고 값이 유지된다. 실질 효과를 보려면 이미지(`meta-cts-wlan`) 쪽 값과 일치시켜야 한다.
- 종전 주석의 "값 자체는 상위 툴 규격으로 고정되어 바꿀 수 없다" 는 서술을 정정했다 — 값은 상위 툴과 합의 대상이고, 접근 범위 제한(`hosts.allow`)은 자격증명으로 대체되지 않는다는 부분만 유지한다.

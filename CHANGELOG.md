...
- **도착값은 `40/65` 로 불변**이다. 출발값 5 종(`70/90`x14, `58/90`, `60/80`, `65/90`, `80/90`)에서 관측된 전환 **18 회 전부**.
...
- `docs/wifi_init_conf_guide.md` §11.5 에 2026-09-14 정정 노트를 추가한다. 기존 2026-08-31 노트("30/50 복귀 미재현")는 그 값·그 자극에 대해서는 여전히 사실이므로 **지우지 않고 남긴다** — 다만 정적 pair 가 유지된다는 뜻으로 읽히지 않도록 위에 새 노트를 둔다.

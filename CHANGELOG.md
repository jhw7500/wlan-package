...
- **기존 기기는 바뀌지 않는다.** `DEBIAN/postinst` 의 JSON deep merge 는 "새 키는 기본값에서 추가, 기존 키는 사용자 값 유지" 규칙이고 `low_thresh`/`high_thresh` 는 이미 모든 배포 기기에 존재한다. 마이그레이션은 **의도적으로 넣지 않았다** — light/중간 트래픽 기기에서는 귀결이 같고, heavy 트래픽 기기에서는 `70/90` 이 살아남으므로 차이가 있을 수 있으나 **그 차이를 우리가 측정하지 못했다**(아래 2026-09-14 정정). 즉 근거 없이 사이트가 의도적으로 넣은 값을 덮어쓰지 않는다는 뜻이다. 종전에 적었던 "기존 기기는 이미 실질적으로 `40/65` 로 동작하고 있어 이득이 없다"는 heavy regime 에서 거짓이므로 철회한다.

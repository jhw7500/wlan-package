...
- **무거운 트래픽에서는 `70/90` 이 덮어써지지 않는다**(경계가 `75<LOW<=80`. 설정 `70/90` 인 heavy arm 은 3 개 모두 무전환이고, 이 절이 비교한 §12.9 의 두 arm 도 각각 무전환). 그 regime 에서 두 설정을 직접 비교한 결과 **선택 rate 가 구분되지 않았다** — `70/90` 45.5300 Mbps {MCS4:108, MCS5:109} vs `40/65` 45.6498 Mbps {MCS4:106, MCS5:111}, 차이 0.1198 Mbps 로 사전 선언한 허용오차 1.39 Mbps 의 약 1/12 다. 두 arm 모두 전환 0 회로 각 설정이 창 전체에서 live 였다(PR #332 Codex P1 지적에 대한 실측 답변). **[정정 2026-09-14, PR #335 Codex P1]** 이 항목은 원래 이를 "regime 별로 각각 확인된 중립"이라고 불렀다. 그 표현은 과대주장이다 — 이 비교는 설정당 1 회 측정이고 허용오차 1.39 가 잡음에 대해 검증되지 않았다. 설정 내 산포를 재 보니(5 번째 캠페인) light 프로파일에서 최대 3.83 Mbps 였고, 이는 관측된 차이 0.1198 보다 훨씬 크다. 따라서 heavy regime 비교는 **미해결(반복 측정 대기)** 이며 두 설정이 같다는 증거도 다르다는 증거도 없다. 상세는 조사 문서 §12.11.
...
- **기존 기기를 전환하려면** 운용자가 `wifi <iface> rate 1 40 65 100` 으로 JSON 을 바꾸고 **재부팅**한다(적용 시점이 boot 이고 연결 중 SET 은 먹지 않는다). 부트 게이트 조건의 light arm(15/15)에서는 전환하지 않아도 동작이 같고 설정 표기만 실제와 달라졌다. heavy 트래픽에서는 `70/90` 이 살아남으므로 **동작이 다를 수 있으며 그 차이는 미측정**이다. 경계 하한이 미측정이므로 경계가 70 위인 세션에서는 light 에서도 동작이 다를 수 있다.
...
출하 기본값 `40/65` 자체는 유지한다 — 이 정정은 착지값·`HIGH` 적용범위에 대한 것이고, 부트 게이트 조건 light arm 의 전환 관측(15/15, 덮어써지므로 그 arm 들에서는 귀결이 같다)에는 영향이 없다. heavy regime 비교의 상태는 아래 2026-09-14 정정을 참조한다.
...
- **출하 기본값을 `40/65` 로 유지하는 근거**: 성능으로는 후보들을 가릴 수 없으므로 남는 근거가
  ⓐ light 에서 관측된 최저 경계(58) 대비 여유가 가장 크고(18 포인트 vs `45/60` 13, `50/70` 8 — 증명된 하한이 아니라 관측값 기준이며 heavy 경계는 `75<LOW<=80` 로 별개),
  ⓑ 전환이 일어나는 조건에서는 FW 자신의 착지값이어서 설정 표기와 실제 동작이 일치하고(heavy 처럼 전환이 없는 조건에서는 착지가 없으므로 이 근거가 적용되지 않는다), ⓒ 설정 내 산포가 가장 작다

...
> SemVer **patch** — 출하 `rate_adapt` 기본값을 `70/90` 에서 `40/65` 로 바꾼다. **두 regime 모두에서 동작 중립임을 측정**했다 — 가벼운 트래픽에서는 FW 가 `70/90` 을 트래픽 2~3 초 뒤 `40/65` 로 덮어쓰고 그 값이 실사용 상태이므로 두 설정의 귀결이 같고, 무거운 트래픽에서는 `70/90` 이 살아남지만 선택 rate 가 `40/65` 와 구분되지 않는다(45.53 vs 45.65 Mbps, 허용오차 1.39). **신규 설치·factory reset 한정** — 기존 기기는 의도적으로 손대지 않는다.
...
- **무거운 트래픽에서는 `70/90` 이 덮어써지지 않는다**(경계가 `75<LOW<=80`, heavy arm 0/2 전환). 그 regime 에서 두 설정을 직접 비교한 결과 **선택 rate 가 구분되지 않았다** — `70/90` 45.5300 Mbps {MCS4:108, MCS5:109} vs `40/65` 45.6498 Mbps {MCS4:106, MCS5:111}, 차이 0.1198 Mbps 로 사전 선언한 허용오차 1.39 Mbps 의 약 1/12 다. 두 arm 모두 전환 0 회로 각 설정이 창 전체에서 live 였다. 따라서 "덮어써지므로 중립"이 아니라 **regime 별로 각각 확인된 중립**이다(PR #332 Codex P1 지적에 대한 실측 답변).
...
출하 기본값 `40/65` 자체는 유지한다 — 두 regime 동작 중립 측정은 이 정정에 영향받지 않는다.

...
제품은 부팅마다 `70/90` 을 pre-association 에 SET 했으나, **가벼운/중간 트래픽에서는** 그 값이
트래픽 2~3초 후 `40/65` 로 덮어써지고 그것이 실사용 상태였다. 그래서 0.6.7 에서 출하 기본값을
`40/65` 로 옮겼다 — 그 조건에서는 설정을 실제 동작에 맞추는 변경이다.

**[정정 2026-09-14, PR #335 Codex P1]** 이 절은 원래 "`70/90` 은 현장 동작을 한 번도 지배한 적이
없다"와 "기존 기기의 동작은 이미 `40/65` 다"라고 무한정으로 적었다. **두 문장 모두 heavy regime
에서 거짓이다** — 경계가 `75<LOW<=80` 인 무거운 트래픽(1000pps/1400B)에서는 `70/90` 이 살아남고
(§12.9 heavy arm 0/2 전환), 그 조건의 기기는 `40/65` 로 동작하지 않는다. 즉 heavy 트래픽을 쓰는
기기에는 이 변경이 **실제 동작 변경**이며, 그 차이의 크기는 §12.9·§12.11 에 따라 **미해결**이다.
"어차피 같으니 무해하다"고 말할 수 없다.

**신규 설치·factory reset 한정**이다. `postinst` 의 JSON deep merge 가 기존 키를 보존하므로 기존
기기는 `70/90` 을 유지한다. 마이그레이션을 넣지 않은 근거는 **"이미 같게 동작하므로 이득이 없다"가
아니라** ⓐ light/중간 트래픽 기기에서는 귀결이 같고 ⓑ heavy 트래픽 기기에서는 차이를 우리가
측정하지 못했으므로 **근거 없이 사이트 값을 덮어쓰지 않는다**는 것이다. 전환하려면
`wifi <iface> rate 1 40 65 100` 후 **재부팅**한다.
...
### 12.9 무거운 트래픽에서의 효과 크기 — 비교 미해결 (정정 2026-09-14)
...
사전 등록한 방향 예측(`mean(40/65) >= mean(70/90)`)은 수치상 성립하지만 차이가 run-to-run 잡음
안이므로 방향 자체는 이 측정으로 **검정되지 않았다** — 방향 주장은 하지 않는다.

**[정정 2026-09-14, PR #335 Codex P1]** 이 절은 원래 "동작 중립은 두 regime 에서 각각 다른
이유로 성립한다"로 끝났다. **무거운 쪽에 대한 그 결론을 철회한다.** 위 허용오차 1.39 는 가벼운
프로파일의 분리력에서 빌려온 값이고 **이 프로파일의 설정 내 산포로 검증되지 않았다** — 설정당
1 회뿐이라 산포를 추정할 수 없었다. 5 번째 캠페인에서 light 프로파일의 설정 내 산포를 실제로
재 보니 최대 **3.83 Mbps** 로, 여기서 관측한 차이 0.1198 보다 훨씬 크다.

`구분되지 않았다` 는 *등가성의 발견*이 아니라 *차이의 증거가 없음*이며, 잡음이 크면 등가 주장이
강해지는 것이 아니라 **관측이 정보를 잃는다**. 따라서 heavy regime 비교는 **미해결(반복 측정
대기)** 이고, 두 설정이 같다는 증거도 다르다는 증거도 없다. 해소에는 heavy 프로파일에서
`70/90`·`40/65` 를 각각 2 회 이상 반복해 산포를 추정해야 한다. 자세한 논거는 §12.11.

가벼운 쪽 결론(`70/90` 이 `40/65` 로 덮어써져 귀결이 같다)은 **전환 관측이므로 영향받지 않는다**.
0.6.7 의 출하 결정은 그 근거만으로 유지된다.
...
0.6.7 의 `40/65` 는 light regime 의 전환 관측(덮어써지므로 귀결이 같다)으로 정당화했는데,
그 근거는 `40/65` 가 FW 착지값이라는 사실에 의존하므로 `45/60` 으로 옮겨가지 않는다. 측정 없이 출하하면 PR #332 에서 지적받은
...
누적으로 `LOW<=55` 설정에서 **전환이 관측된 arm 은 0 개**이고 무전환 arm 은 **15 개**다:
`40/65` x4, `40/90` x3, `45/60` x2, `50/70` x4, `50/90` x1, `55/90` x1 (합 15).
이 중 `heavycfg4065` 1 개만 heavy 프로파일이고 나머지 14 개는 light 다. `45` 는 이번에 처음 시험됐다.

**[정정 2026-09-14]** 이 문단은 원래 "12 arm" 이라고 적었고 내역도 `40/65` x3, `40/90` x2 로
틀렸다(내역 합 13 이 총계 12 와 불일치). PR #335 의 OpenCode 리뷰가 이 불일치를 지적했고,
전 캠페인 표본 시퀀스를 다시 전수 계수해 위 값으로 고쳤다.
...
#### 기존 주장에 대한 영향 — §12.9 heavy regime 비교는 **미해결**로 되돌린다

§12.9 의 heavy regime 비교(허용오차 `d/3`=1.39)는 **설정 내 산포를 측정하지 않았다** —
동일 프로파일 반복 arm 이 없었다. 그 허용오차는 잡음에 대해 검증되지 않았다.

**[정정 2026-09-14, PR #335 Codex P1]** 이 문단은 원래 "결론이 `indistinguishable` 이고 잡음이
크면 그 결론은 더 쉬워지는 방향이므로 결론은 뒤집히지 않는다"고 적었다. **그 추론은 틀렸다.**
`indistinguishable` 은 *등가성의 발견*이 아니라 *차이의 증거가 없음*이다. 잡음이 커지면 등가
주장이 강해지는 것이 아니라 **관측이 정보를 잃는다** — 0.1198 Mbps 차이가 덜 유의미해질 뿐이다.
검증되지 않은 허용오차에 설정당 n=1 이면 heavy regime 에서는 **어느 쪽 증거도 없다**.

따라서 §12.9 의 heavy regime 비교를 **미해결(반복 측정 대기)** 로 되돌린다. 해소에는 heavy
프로파일에서 `70/90` 과 `40/65` 를 각각 2 회 이상 반복해 설정 내 산포를 추정하고, 그 산포가
설정 간 차이보다 작은지 확인해야 한다.

이는 0.6.7 의 출하 결정 근거 중 **heavy regime 항목이 측정으로 뒷받침되지 않는다**는 뜻이다.
light regime 항목(`70/90` 이 `40/65` 로 덮어써져 귀결이 같다)은 전환 관측이므로 영향받지 않고,
그것만으로도 기본값 변경은 유지된다 — heavy regime 에서는 `70/90` 이 살아남지만 그 경우의
성능 차이를 **우리가 측정하지 못했다**는 것이 정확한 상태다.

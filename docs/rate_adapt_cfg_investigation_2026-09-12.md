...
제품은 부팅마다 `70/90` 을 pre-association 에 SET 했으나, **부트 게이트 harness 로 측정한
가벼운 트래픽 arm 에서는** 그 값이 트래픽 2~3초 후 `40/65` 로 덮어써지고 그것이 실사용 상태였다
(fresh FW·배경 9종 정지·연결 전 SET 조건의 light arm 15/15). 그래서 0.6.7 에서 출하 기본값을
`40/65` 로 옮겼다 — **그 조건에서는** 설정을 실제 동작에 맞추는 변경이다.

관측 기반의 한계를 명시한다.
- **중간 트래픽(100pps/56B)은 arm 이 `mid7090` 단 1 개**다. 전환했고 `[40,65]` 로 착지했으나 n=1 이다.
- **무거운 트래픽에서는 전환하지 않는다**(경계 `75<LOW<=80`, heavy arm 0/3). 그 조건에서는
  `70/90` 이 살아남으므로 기본값 변경이 **실제 동작 변경**이고, 차이 크기는 §12.9·§12.11 에 따라 미해결이다.
- **가벼운 트래픽에서도 무조건은 아니다.** 2026-09-12 `rate-trigger-isolation` 의
  `rts-enabled-continuous`·`rts-disabled-continuous` 는 350 개 이상 MPDU 의 지속 송신에도 `70/90` 을
  유지했다. 다만 두 arm 은 부트 게이트 harness 이전이고 fresh FW 가 아니며(uptime 4017~4095 초)
  배경 서비스도 정지하지 않았으므로 **이후 arm 과 직접 비교할 수 없다**. 그래도 "light 면 항상
  전환한다"는 서술의 근거로는 부족하다는 뜻이므로 기록한다.
- 경계는 링크 품질에 따라 움직이고(§12.10 통제 시험) **하한이 미측정**이므로, 경계가 70 위에 앉는
  세션에서는 light 에서도 `70/90` 이 동작을 지배한다. 그 경우 이 변경은 동작 변경이다.
...
(설정 `70/90` 인 heavy arm 3 개 모두 무전환, §12.9 가 비교한 두 arm 도 각각 무전환), 그 조건의 기기는 `40/65` 로 동작하지 않는다. 즉 heavy 트래픽을 쓰는
...
**덮어써지지 않고 살아남으므로**(설정 `70/90` 인 heavy arm 3 개 모두 무전환), 그 regime 의 기기에서는 기본값 변경이 실제
...
부트 게이트 조건 light arm 의 결론(`70/90` 이 `40/65` 로 덮어써져 그 arm 들에서는 귀결이 같다)은 **전환 관측이므로 이 정정에 영향받지 않는다**. 다만 §12.8 의 한계대로 light 라고 항상 그렇지는 않다.
...
0.6.7 의 `40/65` 는 부트 게이트 조건 light arm 의 전환 관측(덮어써지므로 그 arm 들에서는 귀결이 같다)으로 정당화했는데,
...
1. light 에서 관측된 최저 경계(58) 대비 여유가 가장 크다 — 18 포인트(`45/60` 13, `50/70` 8).
   **증명된 하한이 아니라 관측값 기준**이며, heavy 경계(`75<LOW<=80`)는 별개다.
2. **전환이 일어나는 조건에서는** FW 자신의 착지값이어서 설정 표기와 실제 동작이 일치한다.
   `45/60` 은 이 성질을 잃는다. heavy 처럼 전환이 없는 조건에서는 착지가 없으므로 이 근거가 적용되지 않는다.

...
### 12.4 도착값은 `40/65` 로 불변

출발값 5종에서 관측된 전환 **18회 전부** 정확히 `[40, 65]` 로 착지했다(`70/90`×14, `58/90`,
`60/80`, `65/90`, `80/90`). 2절의 과거 `30/50` 관측은 교란·미재현 단일 관측이므로 동급 반례로 보지
않는다.
...
0.6.7 에서 출하 기본값을 `40/65` 로 옮겼다. 동작 변경이 아니라 설정을 실제 동작에 맞추는 변경이며,

...
>
> 그래서 0.6.7에서 출하 기본값을 `70/90`에서 `40/65`로 바꿨다. 근거는 regime별로 세기가 다르다.
> 가벼운 트래픽에서는 `70/90`이 `40/65`로 **덮어써지므로 귀결이 같다** — 전환 관측이라 확정이다.
> 무거운 트래픽에서는 `70/90`이 살아남고(경계 `75<LOW<=80`) 선택 rate가 45.53 vs 45.65 Mbps로
> 나왔으나, **그 비교는 미해결이다** — 설정당 1회 측정이었고 허용오차 1.39가 잡음에 대해 검증되지
> 않았다(§12.11). 즉 heavy regime 에서 두 설정이 같다는 증거도, 다르다는 증거도 없다.
> 기본값 변경은 light regime 근거만으로 유지된다. 상세는 §12.9와 §12.11.
>
...
> 연결 중 SET은 먹지 않는다). 전환하지 않아도 동작은 동일하고, 설정 표기가 실제 동작과
> 다르게 남는 것이 유일한 차이다.
>

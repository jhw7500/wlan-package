...
| `low_thresh` | int | `40` | SR 하한 (%). `255`(0xff)=dynamic |
| `high_thresh` | int | `65` | SR 상한 (%). static은 `low < high`, dynamic은 양쪽 모두 255 |
| `interval_ms` | int | `100` | 양수이며 10ms 배수인 평가 주기 |

0.6.7부터 출하 기본값은 `40/65`다. FW가 정적 pair를 트래픽 중 `40/65`로 덮어쓰고 그 값이 실사용
상태이므로, 기본값을 실제 동작에 맞춘 것이다 — 아래 정정 노트 참조. 확정 상수는 아니며 벤더
회신이나 추가 실기 결과에 따라 다시 조정할 수 있다.
코드 fallback에는 이 수치를 복제하지 않는다.

```json
"mlan0": {
    "rate_adapt": { "enabled": true, "mode": 1, "low_thresh": 40, "high_thresh": 65, "interval_ms": 100 }
},
"mlan1": {
    "rate_adapt": { "enabled": true, "mode": 1, "low_thresh": 40, "high_thresh": 65, "interval_ms": 100 }
}
...
> **[정정 2026-09-14] FW는 정적 pair를 덮어쓴다 — 트리거는 트래픽이다**: 아래 2026-08-31
> 노트의 "30/50 복귀 미재현"은 **그 자극(association 5회)에 대해서만** 사실이다. 값 자체는 실재하며
> 후속 시험에서 `30/50`이 재현됐다(이 절 맨 아래 2026-09-14 추가 정정 참조). 정적 pair가 유지된다는
> 뜻으로 읽으면 안 된다. 2026-09-14 캠페인 3건(총 17 arm, 부팅마다 fresh FW)에서 측정한 바:
>
> - **트리거는 association이 아니라 집계 Tx 트래픽이다.** 송신 개시 2~3초 후 50~60ms 안에 단일
>   계단으로 바뀐다. 아래 노트의 재검증은 자극이 association 5회(부팅·reassociate 2·roam·타AP전환)
>   뿐이었으므로 이 트리거를 만들지 못한다.
> - **도착값은 대개 `40/65`지만 불변은 아니다.** 출발값 5종(`70/90`x14, `58/90`, `60/80`, `65/90`,
>   `80/90`)에서 관측된 전환 18회는 전부 `[40,65]`였으나, 후속 시험에서 `60/90`이 **`[30,50]`**으로
>   착지했다. 전 캠페인 전수 재계수로 전환 21회 = `[40,65]` 20회 + `[30,50]` 1회(전부 단일 계단).
> - **설정 `LOW`가 지배하고 `HIGH`는 무관**하다. 경계는 링크 상태에 따라 움직인다(가벼운 트래픽에서
>   `58<LOW<=60`, 몇 시간 뒤 더 나쁜 링크에서 58 이하, 무거운 트래픽에서 `75<LOW<=80`).
> - **덮어쓴 값은 실사용 상태다.** `70/90`으로 설정해 `40/65`로 덮어써진 인터페이스의 선택 전송
>   rate 평균이 58.66 Mbps로, 진짜 `40/65` 설정(58.11)과 구분되지 않고 `40/90`(53.95)과는 명확히
>   달랐다(사전 선언 허용오차 1.39). 표시만의 문제가 아니다.
> - **dynamic(`0xff/0xff`)과 FW 기본값 상태는 면제**다(967/967 샘플 유지). 정적 pair만 덮어써진다.
> - **연결 중 재설정은 불가**하다. 연결 상태 SET은 출력만 바뀌고 즉시 GET은 덮어쓴 값을 그대로
>   반환한다. 따라서 모니터링 기반 자동 교정은 성립하지 않는다 — 관측·경고 용도로만 쓴다.
>
> 그래서 0.6.7에서 출하 기본값을 `70/90`에서 `40/65`로 바꿨다. **근거는 조건별로 세기가 다르고,
> 어느 조건에서도 "항상 같다"는 아니다.**
>
> - **부트 게이트 조건의 가벼운 트래픽 arm(15/15)에서는** `70/90`이 `40/65`로 덮어써져 귀결이
>   같았다. 전환 관측이므로 그 arm 들에 대해서는 확정이다.
> - **가벼운 트래픽이라고 항상 그렇지는 않다.** 2026-09-12 캠페인의 지속 송신 arm 2개
>   (`rts-enabled-continuous`·`rts-disabled-continuous`)는 350개 이상 MPDU 에도 `70/90`을 유지했다
>   (부트 게이트 이전·warm boot 이라 직접 비교는 불가하되 기록한다). 또 **경계는 링크 품질에 따라
>   움직이고 하한이 미측정**이므로, 경계가 70 위에 앉는 세션에서는 가벼운 트래픽에서도 `70/90`이
>   살아남는다. **그 경우 `40/65`로 바꾸는 것은 실제 동작 변경이다.**
> - **무거운 트래픽에서는** `70/90`이 살아남고(경계 `75<LOW<=80`) 선택 rate가 45.53 vs 45.65 Mbps로
>   나왔으나 **그 비교는 미해결이다** — 설정당 1회 측정이었고 허용오차 1.39가 잡음에 대해 검증되지
>   않았다. 두 설정이 같다는 증거도, 다르다는 증거도 없다.
> - **중간 트래픽(100pps)은 arm 이 1개**(`mid7090`)뿐이다. 전환했으나 n=1 이다.
>
> 즉 기본값 변경의 근거는 "관측된 조건에서 귀결이 같았다"이고 **"모든 조건에서 무해하다"가 아니다.**
> 상세는 조사 문서 §12.8·§12.9·§12.11.
>
> **기존 기기는 의도적으로 바꾸지 않는다(신규 설치·factory reset 한정)**: `postinst`의 JSON deep
> merge는 "기존 키는 사용자 값 유지" 규칙이라 이미 `low_thresh`/`high_thresh`를 가진 기기에는
> 새 기본값이 내려가지 않는다. 기존 기기를 전환하려면 운용자가
> `wifi <iface> rate 1 40 65 100` 으로 JSON을 바꾸고 **재부팅**해야 한다(적용 시점이 boot이며
> 연결 중 SET은 먹지 않는다). 관측된 조건에서는 전환하지 않아도 동작이 같고, 설정 표기가 실제 동작과
> 다르게 남는 것이 유일한 차이다.
>
> 측정 상세는 `docs/rate_adapt_cfg_investigation_2026-09-12.md` §12, 근본 수정은 FW 몫이다
> (벤더 요청서: `docs/rate_adapt_cfg_nxp_request.md`).
>
> **[추가 정정 2026-09-14, 후속 시험]** 4번째 캠페인(`release/rate-txpwr-inverse-20260914-c38b243a`, 4 arm, 연결 전 부트 드롭인,
> Tx 전력 레버)에서 위 두 항목을 정정한다. 상세는 조사 문서 §12.10.
>
> - **착지값은 불변이 아니다**: `60/90` → `[30,50]`(단일 계단, `[40,65]` 경유 없음).
>   같은 `LOW=60`이 `HIGH=80`에서는 `[40,65]`였으므로 **`HIGH` 무관은 전환 여부에만 적용**되고
>   착지값에는 `HIGH`가 관여한다.
> - **경계가 링크 품질을 따른다는 첫 통제 인과 증거**: 같은 `LOW=60`에서 기본 전력(재시도/MPDU
>   0.7225)은 전환, 2 dBm(0.0050)은 무전환 → 경계가 `<60`에서 `60<LOW<=70`으로 올라갔다.
>   종전 근거는 기회적 관측뿐이었다.
> - **설명되지 않음**: `LOW=60`이 `LOW=70`보다 더 빨리 전환했다(0.24s vs 2.90s). 수정 모델은
>   제안하지 않고 벤더 질문으로 넘긴다.
> - **주의(랩 환경 특성)**: 이 근거리 링크(-48 dBm)에서는 Tx 전력을 낮추면 링크가 **개선**된다
>   (기본 20 dBm이 최악). 전력을 열화 수단으로 쓰려는 후속 시험은 이 사실을 먼저 확인할 것.
>
> **[현장 튜닝 2026-09-14]** 출하 기본값은 `40/65`를 유지하되, 운용자가 현장에서 쓸 수 있는
> 값을 5번째 캠페인(`release/rate-tune-4560-20260914-384f81e5`, 4설정 x 2반복)에서 시험했다.
> 상세는 조사 문서 §12.11.
>
> > ⚠️ **먼저 읽을 것 — 아래 결과는 시험한 랩 링크에 한정된다.** 전환 경계는 **링크 품질에 따라
> > 내려간다**(4번째 캠페인에서 통제 시험으로 확인). 관측된 최저 경계는 58이고 **경계 하한은
> > 측정하지 못했다**(Tx 전력으로 링크를 악화시킬 수 없었다). 따라서 현장 링크가 랩보다 나쁘면
> > 아래 값들이 덮어써질 수 있고, **`LOW`를 높게 쓸수록 그 위험이 커진다.** 이 표는
> > "덮어써지지 않는다"는 **보증이 아니다.**
>
> | 설정 | 랩 링크 전환 | 관측 최저 경계(58) 대비 여유 | 설정 내 산포 |
> |---|---|---|---|
> | `40/65` (출하) | 0회 x2 | 18 포인트 | 0.44 Mbps |
> | `45/60` | 0회 x2 | 13 포인트 | 3.83 Mbps |
> | `50/70` | 0회 x2 | **8 포인트 (가장 얇음)** | 2.05 Mbps |
>
> - **시험한 랩 링크에서는 세 값 모두 덮어써지지 않았다**(각 2회 독립 부팅에서 재현).
>   위 경고대로 이것이 현장 보증은 아니다. 적용은 `wifi <iface> rate 1 <low> <high> 100`으로
>   JSON을 바꾸고 **재부팅**한다(연결 중 SET은 먹지 않는다).
> - **성능 차이는 주장하지 않는다.** 사전 등록한 계기 교정이 실패했다 — 같은 설정의 두 측정
>   산포(최대 3.83)가 기준 설정쌍 간격(`40/65` vs `40/90` = 3.47)보다 크다. 60초 창으로는
>   설정 간 성능을 분해할 수 없다. 따라서 `45/60`·`50/70`이 `40/65`보다 빠르다는 근거도,
>   느리다는 근거도 없다.
> - **출하 기본값을 바꿀 이유가 있을 때만 바꾼다.** 여유가 가장 크고(18 포인트) 설정 내 산포가
>   가장 작은 것은 `40/65`이며, FW 자신이 착지하는 값이라 설정 표기와 실제 동작이 일치한다.
>
> **[정정 2026-08-31] 30/50 복귀는 재현되지 않는다** — *이 표제는 그 시점 그 자극에 한정된다.*
> *`30/50` 자체는 2026-09-14 후속 시험에서 재현됐다(위 추가 정정 참조).* 이 절에는 "mlan0 직접 roam의 `COMPLETED`에서

...
>
> 그래서 0.6.7에서 출하 기본값을 `70/90`에서 `40/65`로 바꿨다. **두 regime 모두에서 동작 중립임을
> 측정했다** — 가벼운 트래픽에서는 `70/90`이 `40/65`로 덮어써져 귀결이 같고, **무거운 트래픽에서는
> `70/90`이 살아남지만**(경계 `75<LOW<=80`) 선택 rate가 `40/65`와 구분되지 않는다(45.53 vs
> 45.65 Mbps, 사전 선언 허용오차 1.39, MCS 분포도 사실상 동일). 상세는 조사 문서 §12.9.
>
...
> 값을 5번째 캠페인(`release/rate-tune-4560-20260914-384f81e5`, 4설정 x 2반복)에서 검증했다.
> 상세는 조사 문서 §12.11.
>
> | 설정 | 전환 | 관측 최저 경계(58) 대비 여유 | 설정 내 산포 |
> |---|---|---|---|
> | `40/65` (출하) | 0회 x2 | 18 포인트 | 0.44 Mbps |
> | `45/60` | 0회 x2 | 13 포인트 | 3.83 Mbps |
> | `50/70` | 0회 x2 | 8 포인트 | 2.05 Mbps |
>
> - **세 값 모두 덮어써지지 않는다**(각 2회 독립 부팅에서 재현). `wifi <iface> rate 1 <low> <high> 100`
>   으로 JSON을 바꾸고 **재부팅**하면 적용된다(연결 중 SET은 먹지 않는다).
> - **성능 차이는 주장하지 않는다.** 사전 등록한 계기 교정이 실패했다 — 같은 설정의 두 측정
>   산포(최대 3.83)가 기준 설정쌍 간격(`40/65` vs `40/90` = 3.47)보다 크다. 60초 창으로는
>   설정 간 성능을 분해할 수 없다.
> - **`LOW`를 올릴수록 경계에 가까워진다.** 관측된 최저 전환 경계는 58이고 경계는 링크 품질에
>   따라 내려간다. 현장 링크가 랩보다 나쁘면 `50/70`이 덮어써질 수 있다 — **경계 하한은
>   미측정이다**(Tx 전력으로 링크를 악화시킬 수 없어 시험하지 못했다). `LOW`를 높게 쓸수록
>   이 위험이 커진다.
>
